...
class BloodRequestImpl extends BloodRequestRepo {
  ...
  @override
  Future<Either<Failure, List<BloodRequest>>> fetchMyBloodrequests() async {
    try {
      final allRequests = await requestRef
          .where("requestedBy", isEqualTo: userId)
          .get();

      if (allRequests.docs.isNotEmpty) {
        final data = allRequests.docs
            .map((e) => BloodRequest.fromJson(e.data()))
            .toList();

        for (final req in data) {
          await MyBloodRequestHiveManager.addRequest(req);
        }
        return Right(data);
      } else {
